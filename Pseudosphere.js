...
function start1 () {
    ...
    gl.vertexAttribPointer(colAttrib, 4, gl.FLOAT, false, 28, 12);
    gl.vertexAttrib4f(colAttrib, 0.7, 0.5, 0.1, 1);
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, iboTris);
...
function createVertexDataPseudosphere(){
    ...
    for(var i=0, u=rangeU.min; i <= n; i++, u += du) {
        ...
        for(var j=0, v=rangeV.min; j <= m; j++, v += dv) {
            ...
            var x = c * (Math.cos(u)*Math.sin(v));
            var z = c * (Math.sin(u)*Math.sin(v));
            var y = c * (Math.cos(v)+Math.log(Math.tan(v/2)));
            ...
            //Farbe
            vertices[iVertex * 3 + 3] = 1;
            vertices[iVertex * 3 + 4] = 0.1;
            vertices[iVertex * 3 + 5] = 0;
            vertices[iVertex * 3 + 6] = 1;
