...
function createVertexDataTower(){
    var n = 16;
    var m = 46;
...
const rangeV = { min: (0.001), max: (1.7) };
...
for(var i=0, u=rangeU.min; i <= n; i++, u += du) {
    ...
    for(var j=0, v=rangeV.min; j <= m; j++, v += dv) {
            ...
            var x = c * (Math.cos(u)*(3*Math.cos(v)-Math.cos(3*v)));
            var z = c * (Math.sin(u)*Math.sin(v));
            var y = c * (2+Math.cos(v*3)+Math.log(Math.tan(v))*1.7);
